import { Plus, RotateCcw, Search, Trash2, Upload } from "lucide-react";
import { useEffect, useState, type ChangeEvent } from "react";
import { Link, useNavigate } from "react-router-dom";

import type { InfiniteCanvas } from "../types/canvas";

import { normalizeCanvasImport } from "../lib/canvas-compat.js";
import {
    createCanvasApi,
    importCanvasApi,
    listCanvasesApi,
    restoreCanvasApi,
    trashCanvasApi,
} from "../stores/canvas-store";
import type { CanvasKind } from "../types/canvas";

export default function CanvasListPage() {
    const navigate = useNavigate();
    const [keyword, setKeyword] = useState("");
    const [showTrash, setShowTrash] = useState(false);
    const [version, setVersion] = useState(0);
    const [canvases, setCanvases] = useState<InfiniteCanvas[]>([]);

    useEffect(() => {
        listCanvasesApi(showTrash).then((list) => {
            setCanvases(list.filter((c) => c.title.toLowerCase().includes(keyword.toLowerCase())));
        });
    }, [keyword, showTrash, version]);

    function create(kind: CanvasKind) {
        createCanvasApi(kind, kind === "smart" ? "智能画布" : "经典画布").then((canvas) => {
            navigate(`/${kind === "smart" ? "smart-canvas" : "canvas"}/${canvas.id}`);
        });
    }

    function importFile(event: ChangeEvent<HTMLInputElement>) {
        const file = event.target.files?.[0];
        if (!file) return;
        const reader = new FileReader();
        reader.onload = () => {
            const normalized = normalizeCanvasImport(JSON.parse(String(reader.result)));
            importCanvasApi(normalized).then((canvas) => {
                navigate(`/${canvas.kind === "smart" ? "smart-canvas" : "canvas"}/${canvas.id}`);
            });
        };
        reader.readAsText(file);
    }

    return (
        <main className="ic-page">
            <div className="ic-page-head">
                <div>
                    <h1>画布管理</h1>
                    <p>管理经典画布、智能画布和原项目兼容 JSON。</p>
                </div>
                <div className="ic-actions">
                    <button type="button" onClick={() => create("classic")}><Plus size={16} /> 经典画布</button>
                    <button type="button" onClick={() => create("smart")}><Plus size={16} /> 智能画布</button>
                    <label className="ic-icon-button">
                        <Upload size={16} /> 导入
                        <input type="file" accept="application/json" onChange={importFile} />
                    </label>
                </div>
            </div>
            <div className="ic-filter">
                <Search size={16} />
                <input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="搜索画布标题" />
                <button type="button" onClick={() => setShowTrash((value) => !value)}>
                    {showTrash ? "隐藏回收站" : "显示回收站"}
                </button>
            </div>
            <div className="ic-card-grid">
                {canvases.length === 0 && <p className="ic-muted" style={{ gridColumn: "1/-1", textAlign: "center", padding: 40 }}>没有匹配的画布。</p>}
                {canvases.map((canvas) => (
                    <Link key={canvas.id} to={"/" + (canvas.kind === "smart" ? "smart-canvas" : "canvas") + "/" + canvas.id}
                        className={"ic-card-item" + (canvas.deletedAt ? " is-deleted" : "")}>
                        <div className="ic-card-preview">
                            {canvas.kind === "smart" ? "\u26a1" : "\ud83d\udcbc"}
                        </div>
                        <div className="ic-card-actions">
                            {canvas.deletedAt ? (
                                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); restoreCanvasApi(canvas.id); setVersion((v) => v + 1); }}
                                    title="恢复"><RotateCcw size={13} /></button>
                            ) : (
                                <button className="danger" onClick={(e) => { e.preventDefault(); e.stopPropagation(); trashCanvasApi(canvas.id); setVersion((v) => v + 1); }}
                                    title="删除"><Trash2 size={13} /></button>
                            )}
                        </div>
                        <div className="ic-card-info">
                            <span className="ic-card-title">{canvas.title}</span>
                            <span className="ic-card-meta">{canvas.kind === "smart" ? "智能" : "经典"} / {new Date(canvas.updatedAt).toLocaleDateString()}</span>
                        </div>
                    </Link>
                ))}
            </div>
        </main>
    );
}
