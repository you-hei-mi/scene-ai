...
本文基于当前仓库源码、根配置、`package.json`、`docker-compose.yml`、`ecosystem.config.js`、`.env.example`、`packages/api` 与 `apps/web-nuxt` 的实际实现编写，适用于 BuildingAI `26.1.1` 版本在本地开发环境与虚拟机环境中的部署、调试、运维和故障恢复。
...
```mermaid
flowchart LR
    DevIDE["本地 IDE"] --> WebDev["前端开发服务 pnpm dev:web 3000"]
...
3. 前端位于 `apps/web-nuxt`，使用 Nuxt 4 + Vue 3，开发端口为 `3000`。
...
```bash
curl -i -H "Origin: http://localhost:3000" http://192.168.232.100:4090/consoleapi/health
```
