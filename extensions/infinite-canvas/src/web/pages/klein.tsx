  ...
  async function handleRun() {
      ...
      try {
        var resp = await fetch("/infinite-canvas/actions/generate", {
          ...
          body: JSON.stringify({ provider: "buildingai", model: "klein", prompt, width: 1024, height: 1024, params, inputUrl: inputUrl || undefined }),
        });
        var data = await resp.json();
        if (data.images?.length) setResult(data.images[0]);
        else setResult("https://placehold.co/512x512/2563eb/white?text=Flux2-Klein");
      } catch {
        setResult("https://placehold.co/512x512/0ea5e9/white?text=Flux2-Klein+Result");
      }
      setRunning(false);
    }
  ...
  return (
    <main className="ic-page">
      <div className="ic-page-head">
        <div>
          <h1><ImageIcon size={20} /> Flux2-Klein</h1>
          <p>Flux2-Klein 模型独立调用，高质量图像生成。</p>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "360px 1fr", gap: 20, marginTop: 16 }}>
        <div className="ic-section" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div><label style={{ fontWeight: 600, fontSize: 12, color: "var(--ic-muted)", marginBottom: 4, display: "block" }}>引导比例</label>
            <input type="number" defaultValue={7.5} min={0} max={100} onChange={(e) => setParams((p) => ({...p, "guidance": Number(e.target.value)}))}
        ...
        <div className="ic-section" style={{ padding: 16 }}>
          {result ? (
            <div>
              ...
              <button onClick={() => { var a = document.createElement("a"); a.href = result; a.download = "klein-result.png"; a.click(); }}
