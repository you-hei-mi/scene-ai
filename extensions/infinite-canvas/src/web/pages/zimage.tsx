  ...
  async function handleRun() {
      ...
      try {
        var resp = await fetch("/infinite-canvas/actions/generate", {
          ...
          body: JSON.stringify({ provider: "buildingai", model: "zimage", prompt, width: 1024, height: 1024, params, inputUrl: inputUrl || undefined }),
        });
        var data = await resp.json();
        if (data.images?.length) setResult(data.images[0]);
        else setResult("https://placehold.co/512x512/2563eb/white?text=Z-Image+生成");
      } catch {
        setResult("https://placehold.co/512x512/0ea5e9/white?text=Z-Image+生成+Result");
      }
      setRunning(false);
    }
  ...
  return (
    <main className="ic-page">
      <div className="ic-page-head">
        <div>
          <h1><ImageIcon size={20} /> Z-Image 生成</h1>
          <p>Z-Image 主模型调用，文生图和图生图。</p>
        </div>
      </div>
        ...
        <div className="ic-section" style={{ padding: 16 }}>
          {result ? (
            <div>
              ...
              <button onClick={() => { var a = document.createElement("a"); a.href = result; a.download = "zimage-result.png"; a.click(); }}
